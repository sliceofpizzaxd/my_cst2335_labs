    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `ToDo` (`id` INTEGER NOT NULL, `entry` TEXT NOT NULL, PRIMARY KEY (`id`))');
  ...
  _$ToDoDAO(
    ...
    this.changeListener,
        ...
        _toDoInsertionAdapter = InsertionAdapter(
            ...
            'ToDo',
            (ToDo item) =>
                <String, Object?>{'id': item.id, 'entry': item.entry}),
  ...
  Future<List<ToDo>> getList() async {
    return _queryAdapter.queryList('SELECT * FROM ToDo',
        mapper: (Map<String, Object?> row) =>
            ToDo(row['entry'] as String, id: row['id'] as int?));
  }

  @override
  Future<void> insertItem(ToDo item) async {
    await _toDoInsertionAdapter.insert(item, OnConflictStrategy.abort);
  }

  @override
  Future<void> deleteItem(ToDo item) async {
